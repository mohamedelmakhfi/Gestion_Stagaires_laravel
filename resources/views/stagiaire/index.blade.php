...
<body class="app sidebar-mini rtl">
    ...
    <main class="app-content">
        ...
        <div class="app-title">
            <div>
                <h1><i class="fa fa-th-list"></i> Liste des stagiaires</h1>
            </div>
            <ul class="app-breadcrumb breadcrumb side">
                <li class="breadcrumb-item"><i class="fa fa-home fa-lg"></i></li>
                <li class="breadcrumb-item">Stagiaire</li>
                <li class="breadcrumb-item active"><a href="/stagiaires">Liste des stagiaires</a></li>
            </ul>
        </div>
                            ...
                            <tbody>
                                @foreach ($stagiaires as $stagiaire)
                                    <tr>
                                        ...
                                        <td><img src="{{ 'uploads/stagiaires/' . $stagiaire->photo }}" width="90px"
                                                height="90px"></td>
                                        <td>{{ $stagiaire->statut }}</td>
                                        <td><a href="{{ route('stage.create') }}" class="btn btn-primary">+</a></td>
                                        <td><button class="btn btn-info"> <a target="_blank"
                                                    href="{{ asset('uploads/cv/' . $stagiaire->cv) }}"
                                                    ...
                                                    </svg>&nbsp; Voir CV</a></button></td>

                                        <td class="d-flex justify-content-between">
                                            <div class="d-flex">
                                                <a href="{{ route('stagiaire.change', $stagiaire->id) }}"
                                                    class="edit-link">
                                                    <img src="/images/edit.png" alt="Modifier"
                                                        class="edit-icon d-inline">
                                                </a>
                                                <form action="{{ route('stagiaire.destroy', $stagiaire->id) }}"
                                                    method="POST" id="delete-stagiaire-form">
                                                    @csrf
                                                    @method('DELETE')
                                                    <input type="image" src="/images/delete.png" alt="Supprimer"
                                                        class="delete-icon d-inline"
                                                        onclick="return confirm('Voulez-vous vraiment supprimer ce stagiaire ?')">
                                                </form>

                                                <form action="{{ route('stagiaire.archive', $stagiaire->id) }}" method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <button type="submit" class="destroy-link">
                                                        <img src="/images/archiver.png" alt="Archiver" class="archive-icon d-inline">
                                                    </button>
                                                </form>
